...
flags.DEFINE_string(
    "config_path", "./configs/sac_atari_config.yaml", "Path to the config"
)
...
def train(config: dict):
    env = AtariPreprocessing(gym.make("ALE/Breakout-v5", frameskip=1))
    ...
    agent = SAC(config, get_atari_networks)
    buffer = ReplayBuffer(
        config["seed"],
        config["buffer_size"],
